...
//IMPORTANT NOTE: LRU LOGIC: head of list is the most recent and the end of the list is the least recent used
...
block_t find_key(const char *uri, cache_t *cache){
    ...
    for (currBlock; currBlock != cache->tail; currBlock = currBlock->next){
...
/*insert_block: insert new URI at the front and if there is not enough size to insert(remove the last block)*/
...
void remove_block(cache_t *cache){
    ...
    if (rBlock == NULL || cache == NULL) return;
