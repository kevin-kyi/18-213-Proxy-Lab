...
/*Cache Implementation: doubly linked list with each block containing data on URI and URI data
With LRU structure of most recent: head of cache & least recent: tail of cache*/
...
/*remove_block: removes the block from the tail of the cache(least recently used block)*/
void remove_block(cache_t *cache);


/*update_LRU: If looking through cache for URI and finds one then moves that block to the head(most recently used block)*/
